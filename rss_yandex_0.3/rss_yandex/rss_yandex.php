...
/* 		if ($truncateLen > 0) {
			...
			if (is_array($xfd = xf_decode($row['xfields'])) && isset($xfd[pluginGetVariable('rss_yandex', 'xfEnclosure')])) {
				...
				if ($enclosureIsImages) {
					...
					if (isset($encImages[$row['id']])) {
						$img = '' . ($encImages[$row['id']]['storage'] ? $config['attach_url'] : $config['images_url']) . '/' . $encImages[$row['id']]['folder'] . '/' . $encImages[$row['id']]['name'] . '';
						$fh = fopen($img, "r");
						while(($str = fread($fh, 1024)) != null) $fsize += strlen($str);
						$enclosureList [] = '   <enclosure url="'.$img.'" type="image/'.pathinfo($img, PATHINFO_EXTENSION).'" length="' .$fsize.'" />';
					}
				} else {
					// text
		...
		if (pluginGetVariable('rss_yandex', 'textEnclosureEnabled') && isset($newsVars['news']['embed']['images']) && is_array($newsVars['news']['embed']['images'])) {
			foreach ($newsVars['news']['embed']['images'] as $url) {
				...
				$es = fopen($url, "r");
				while(($stur = fread($es, 1024)) != null) $esize += strlen($stur);
				$enclosureList [] = '   <enclosure url="' . $url . '" type="image/'.pathinfo($url, PATHINFO_EXTENSION).'" length="' .$esize.'" />';
			}
		}
		...
		$output .= "   <yandex:full-text>" . stripslashes(htmlspecialchars($content)) . "</yandex:full-text>\n";
...
function plugin_rss_yandex_mk_header($xcat) {
	...
	$line .= " <channel>\n";
	// Channel title
	$line .= "  <title><![CDATA[" . $feedTitleFormat/* $config['home_title'] . (is_array($xcat) ? ' :: ' . $xcat['name'] : '') */ . "]]></title>\n";
	// LINK
	...
	$line .= " </image>\n";
	$line .= "  <generator><![CDATA[Plugin rss_yandex (0.03) // Next Generation CMS (" . engineVersion . ")]]></generator>\n";
