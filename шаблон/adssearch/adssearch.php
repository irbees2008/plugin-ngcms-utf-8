...
register_plugin_page('adssearch','','adssearch');
...
function adssearch() {//берем основные данные
...
$cacheFileName = adspricefrom(3).'_'.adspricefor(3).'_'.adsquarefrom(3).'_'.adsquarefor(3).'_'.numpage().'_'.md5('adssearch'.$config['theme'].$config['default_lang'].$year.$month).'.txt';


		if (extra_get_param('adssearch','cache')){
		$cacheData = cacheRetrieveFile($cacheFileName, extra_get_param('adssearch','cacheExpire'), 'adssearch');
		if ($cacheData != false)
			{
...
//??
    $num = intval(extra_get_param('adssearch','number'));
        if (($num < 1) || ($num > 50)) {$num = 10;}    
 
    $tpath = locatePluginTemplates(array('adssearch', 'entries'), 'adssearch', 1);
//??

	foreach ($mysql->select("select postdate, title, alt_name, catid, xfields_indexpreview, xfields_idobj, xfields_adres, xfields_ploshad, xfields_etaj, xfields_dometro, xfields_indexanons, xfields_price from ".uprefix."_news where approve = '1' AND xfields_idobj !='' ".adspricefrom(1)." ".adspricefor(1)." ".adsquarefrom(1)." ".adsquarefor(1)." order by xfields_price ASC LIMIT ".numpage().", 10 ") as $row) {
...
$anons_display = '<br />'.$row['xfields_indexanons'];
        ...
        $tpl -> template('adssearch', $tpath['adssearch']);
        $tpl -> vars('adssearch', $tvars);
        $output .= $tpl -> show('adssearch');
		$template['vars']['mainblock'] = $output;


//
		if (extra_get_param('adssearch','cache')) {
		cacheStoreFile($cacheFileName, $output, 'adssearch');
		}
//
...
function adstypes() {//результаты begin
	...
	$adspricetype = 'цена: '.adspricefrom(2).adspricefor(2).' руб.';
	}else{
...
$tpl -> template('adstypes', extras_dir."/adssearch/tpl");
...
$counts = mysql_result(mysql_query("SELECT count(*) FROM ".uprefix."_news WHERE approve = '1' AND xfields_idobj !='' ".adspricefrom(1)." ".adspricefor(1)." ".adsquarefrom(1)." ".adsquarefor(1)." "),0);
//print $counts;
return $counts;
}
...
if (listcounts() > 10){
  ...
  for ($i=0; $i<$chislo; $i++){
	...
	// !preg_match("/^[0-9]+$/", $apage)
		...
		$p_link .= '<a href="http://metroskop.ru/plugin/adssearch/';
...
$tpl -> template('pagination', extras_dir."/adssearch/tpl");
