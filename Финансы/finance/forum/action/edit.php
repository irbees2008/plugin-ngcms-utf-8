<?php
	...
	*/
	...
	$sql = 'SELECT p.message, p.author_id, p.tid, p.id, p.c_data, t.title FROM '.prefix.'_forum_posts AS p LEFT JOIN '.prefix.'_forum_topics AS t ON t.id = p.tid WHERE p.id = '.securemysql($id).' LIMIT 1';
	...
	$edit_subject = ($id == $row_2['id'] && $userROW['status'] == 1)?1:0;
