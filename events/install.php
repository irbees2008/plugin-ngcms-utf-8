...
function plugin_events_install($action) {
	global $lang, $mysql;
	
	if(!file_exists(dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/events'))
		if(!@mkdir(dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/events/', 0777))
			msg(array("type" => "error", "text" => "Критическая ошибка <br /> не удалось создать папку ".dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/images/events'), 1);
	
	if(!file_exists(dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/events/thumb'))
		if(!@mkdir(dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/events/thumb', 0777))
			msg(array("type" => "error", "text" => "Критическая ошибка <br /> не удалось создать папку ".dirname(dirname(dirname(dirname(__FILE__)))).'/uploads/images/events/thumb'), 1);
	...
	array(
		...
		'action'	=> 'cmodify',
		'engine'	=> 'MyISAM',
		'key'		=> 'primary key(id), KEY `cat_id` (`cat_id`), KEY `active` (`active`), KEY `events_view` (`views`), FULLTEXT (announce_name), FULLTEXT (announce_description)',
		'fields'	=> array(
			...
			array('action'	=> 'cmodify', 'name' => 'announce_place', 'type' => 'varchar(255)', 'params' => 'NOT NULL default \'\''),
			...
			array('action'	=> 'cmodify', 'name' => 'city', 'type' => 'int(10)', 'params' => 'NOT NULL default \'0\''),
			array('action'	=> 'cmodify', 'name' => 'gender', 'type' => 'varchar(1)', 'params' => 'NOT NULL default \'\''),
	...
	{
		case 'confirm':
			generate_install_page('events', file_get_contents(''));break;
		case 'autoapply':
		case 'apply':
			if (fixdb_plugin_install('events', $db_update, 'install', ($action=='autoapply')?true:false)) {
			
					if(!$mysql->record('SHOW INDEX FROM '.prefix.'_events WHERE Key_name = \'announce_name\''))
						$mysql->query('alter table '.prefix.'_events add FULLTEXT (announce_name)');
					
					if(!$mysql->record('SHOW INDEX FROM '.prefix.'_events WHERE Key_name = \'announce_description\''))
						$mysql->query('alter table '.prefix.'_events add FULLTEXT (announce_description)');
			
				plugin_mark_installed('events');
			} else {
				return false;
			}
