...
register_plugin_page('adssearch2','','adssearch2');
...
function adssearch2() {//берем основные данные
...
$cacheFileName = adspricefrom(3).'_'.adspricefor(3).'_'.adsquarefrom(3).'_'.adsquarefor(3).'_'.numpage().'_'.md5('adssearch2'.$config['theme'].$config['default_lang'].$year.$month).'.txt';


		if (extra_get_param('adssearch2','cache')){
		$cacheData = cacheRetrieveFile($cacheFileName, extra_get_param('adssearch2','cacheExpire'), 'adssearch2');
		if ($cacheData != false)
			{
...
//??
    $num = intval(extra_get_param('adssearch2','number'));
        if (($num < 1) || ($num > 50)) {$num = 10;}    
 
    $tpath = locatePluginTemplates(array('adssearch2', 'entries'), 'adssearch2', 1);
//??

	foreach ($mysql->select("select postdate, title, alt_name, catid, xfields_indexpreview, xfields_idobj, xfields_adres, xfields_ploshad, xfields_etaj, xfields_region, xfields_indexanons, xfields_price from ".uprefix."_news where approve = '1' AND xfields_idobj !='' AND catid IN ('77,78', '77,79', '77,80') ".adspricefrom(1)." ".adspricefor(1)." ".adsquarefrom(1)." ".adsquarefor(1)." order by xfields_price ASC LIMIT ".numpage().", 10 ") as $row) {
...
$anons_display = '<br />'.$row['xfields_indexanons'];
        ...
        $tpl -> template('adssearch2', $tpath['adssearch2']);
        $tpl -> vars('adssearch2', $tvars);
        $output .= $tpl -> show('adssearch2');
		$template['vars']['mainblock'] = $output;


//
		if (extra_get_param('adssearch2','cache')) {
		cacheStoreFile($cacheFileName, $output, 'adssearch2');
		}
//
...
$tpl -> template('adstypes', extras_dir."/adssearch2/tpl");
...
$counts = mysql_result(mysql_query("SELECT count(*) FROM ".uprefix."_news WHERE approve = '1' AND xfields_idobj !='' AND catid IN ('77,78', '77,79', '77,80') ".adspricefrom(1)." ".adspricefor(1)." ".adsquarefrom(1)." ".adsquarefor(1)." "),0);
//print $counts;
return $counts;
}
...
if (listcounts() > 10){
  ...
  for ($i=0; $i<$chislo; $i++){
	...
	// !preg_match("/^[0-9]+$/", $apage)
		...
		$p_link .= '<a href="http://metroskop.ru/plugin/adssearch2/';
...
$tpl -> template('pagination', extras_dir."/adssearch2/tpl");
