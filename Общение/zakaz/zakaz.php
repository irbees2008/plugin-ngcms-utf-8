...
if ((!$mode)||($mode<2))
{
 ...
 $captcha = '';
	...
	$tvars['regx']['#\[error\](.*?)\[\/error\]#is']	= '';
		...
		$tvars['vars']['zakazmes']=secure_html($_REQUEST['zakazmes']).(($errorText == '')?'':' Файл '.$errorText);
...
function plugin_zakaz_post() 
{
	...
	$SYSTEM_FLAGS['info']['title']['item']	= str_replace('{title}','Заказ', $lang['zakaz:header.send']);
		...
		if(empty($_REQUEST['zname'])||strlen($_REQUEST['zname'])<2||!preg_match('/^[А-Яа-я\s]+$/', $_REQUEST['zname']))
		{
		...
		if ($_FILES['attach']['name'])
		{
			...
			{
				@unlink($ftmp);
				plugin_zakaz_showScreen(1, $lang['zakaz:uplmaxsize'].extra_get_param('zakaz', 'maxf').' Кб');
				return;
			}
	...
	$mailBody = '';
		$tmvars = array('vars' => array(
					...
					'zakazmes'			=>$zmes.(($_FILES['attach']['name'])?'<br>Файл '.$fname:''),
