...
images: [{% for entry in entriesImg %}{
image: "{{ entry.home }}/uploads/zboard/{{ entry.filepath }}",
alt: "{{ entry.filepath }}",
text: "{{ entry.filepath }}",
...
<div class="post clearfix">
	<div class="entry">
		...
		<div class="desc">{{date|date("m-d-Y H:i")}}
			/
			{{announce_author}}</div>
		<div class="view">
...
<div class="text">
	...
	{% set tel_clean = announce_contacts|replace({' ': '', '(': '', ')': '', '-': ''}) %}
	{% if tel_clean starts with '+' %}
		<p>
			<a href="tel:{{ tel_clean }}">{{ announce_contacts }}</a>
		</p>
	{% else %}
