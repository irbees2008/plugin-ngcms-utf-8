...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="{{ lang['langcode'] }}" lang="{{ lang['langcode'] }}" dir="ltr">
	<head>
		...
		<link
		rel="stylesheet" href="{{ tpl_url }}/css/mangguo.css">
	...
	<body>
		...
		<div class="page">
			<div class="header">
				...
				<ul
					class="quick-menu clearfix">
					<!-- <li><span>RSS</span></li> -->
					{% if not (global.flags.isLogged) %}
						<li class="subscribe">
							<a href="{{ home }}/login/">Вход</a>
						</li>
						<li class="random">
...
{{ callPlugin('zboard.show', {'number' : 5, 'mode' : 'last', 'cat': '1,2,3', 'template': 'block_zboard'}) }}
					...
					<div class="search">
						<form action="{{home}}/zboard/search/" method="get" class="searchform clearfix">
							<span class="s"><input type="text" class="form-control" name="keywords" id="keywords" value=""></span>
					...
					<div class="tab">
						<ul class="clearfix">
							<li class="menu-item menu-item-type-custom menu-item-object-custom {% if isHandler('news') %}current-menu-item{% endif %}">
								<a href="{{home}}">Главная</a>
							</li>
							<li class="menu-item menu-item-type-post_type menu-item-object-page {% if isHandler('zboard') and not isHandler('zboard:list') and not ( isHandler('zboard:send') or isHandler('zboard:edit') ) %}current-menu-item{% endif %}">
								<a href="{{home}}/plugin/zboard/">Доска объявлений</a>
							</li>
							<li class="menu-item menu-item-type-custom menu-item-object-custom {% if isHandler('zboard:send') %}current-menu-item{% endif %}">
								<a href="{{home}}/plugin/zboard/send/">Добавить объявление</a>
							</li>
							{% if (global.flags.isLogged) %}
								<li class="menu-item menu-item-type-post_type menu-item-object-page {% if (global.flags.isLogged) and ( isHandler('zboard:list') or isHandler('zboard:edit') ) %}current-menu-item{% endif %}">
									<a href="{{home}}/plugin/zboard/list/">Список моих объявлений</a>
								</li>
							{% endif %}
						</ul>
					</div>
					<div class="quick-link">
						<div class="clearfix">
							<div class="counter">Всего доступно объявлений
								<a href="{{home}}/plugin/zboard/">
									<b>{{ callPlugin('zboard.show_entries_cnt', {}) }}</b>
								</a>
							</div>
						</div>
					</div>
			...
			<div class="footer">
				<div class="copyright">COPYRIGHT &copy; 2010-{{ now|date("Y") }}
					<a href="http://ngcms.org">NG CMS</a>
					- THEME BY
					<a target="_blank" href="http://www.mangguo.org/">MANGGUO.ORG</a>
				</div>
