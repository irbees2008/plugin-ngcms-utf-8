...
function plugin_block_zboard($number, $mode, $cat, $overrideTemplateName, $cacheExpire)
{
	...
	switch ($mode) {
		case 'view':
			$sql = 'SELECT *, c.id as cid, n.id as nid FROM ' . prefix . '_zboard n LEFT JOIN ' . prefix . '_zboard_cat c ON n.cat_id = c.id LEFT JOIN ' . prefix . '_zboard_images i ON n.id = i.zid WHERE n.active = \'1\' ' . $sorting . ' GROUP BY n.id ORDER BY n.views DESC';
			break;
		case 'last':
		...
		case 'rnd':
			...
			break;
		case 'vip':
			$sql = 'SELECT *, c.id as cid, n.id as nid FROM ' . prefix . '_zboard n LEFT JOIN ' . prefix . '_zboard_cat c ON n.cat_id = c.id LEFT JOIN ' . prefix . '_zboard_images i ON n.id = i.zid WHERE n.active = \'1\' ' . $sorting . ' AND n.vip_expired != "" ORDER BY n.vip_expired DESC';
			break;
		default:
			$mode = 'last';
			$sql = 'SELECT *, c.id as cid, n.id as nid FROM ' . prefix . '_zboard n LEFT JOIN ' . prefix . '_zboard_cat c ON n.cat_id = c.id LEFT JOIN ' . prefix . '_zboard_images i ON n.id = i.zid WHERE n.active = \'1\' ' . $sorting . ' GROUP BY n.id ORDER BY editdate DESC';
