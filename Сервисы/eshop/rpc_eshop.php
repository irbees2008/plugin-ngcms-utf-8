...
function linked_prd($params)
{
    ...
    $sqlQPart = "FROM ".prefix."_eshop_products p LEFT JOIN ".prefix."_eshop_products_categories pc ON p.id = pc.product_id LEFT JOIN ".prefix."_eshop_categories c ON pc.category_id = c.id LEFT JOIN ".prefix."_eshop_images i ON i.product_id = p.id ".(count(
            $conditions
...
function change_img_pos($params)
{
    ...
    if ($userROW['status'] < 3) {
        ...
        if (isset($img_row)) {
            $mysql->query(
                "update ".prefix."_eshop_images set position = ".db_squote(
                    $img_row['position']
                )." where (product_id = ".db_squote($img_row['product_id']).") and (position = ".db_squote(
                    $position
                ).") "
            );
            $mysql->query(
                "update ".prefix."_eshop_images set position = ".db_squote($position)." where (id = ".db_squote(
                    $img_row['id']
                ).") "
            );
            ...
            return array(
                'status' => 1,
                'errorCode' => 0,
                'data' => iconv("windows-1251", "utf-8", 'Images swaped success'),
                'update' => '',
            );
        }
    }
    ...
    return array(
        'status' => 0,
        'errorCode' => 0,
        'data' => iconv("windows-1251", "utf-8", 'OK, '.var_export($params, true)),
    );
}
...
function change_variant($params)
{
    ...
    $value = (float)filter_var($params['value'], FILTER_SANITIZE_STRING);
    ...
    if ($userROW['status'] < 3) {
        ...
        if (isset($qid)) {
            ...
            $mysql->query(
                'UPDATE '.prefix.'_eshop_variants SET '.implode(', ', $vnames).' WHERE id = \''.intval($qid).'\'  '
            );

            return array(
                'status' => 1,
                'errorCode' => 0,
                'data' => iconv("windows-1251", "utf-8", 'Variant success changed'),
                'update' => '',
            );
        }
...
function compare_prd($params)
{
    ...
    if ($id < 1) {
        return array(
            'status' => 0,
            'errorCode' => 2,
            'errorText' => iconv("windows-1251", "utf-8", 'ID should be positive'),
        );
    }
    ...
    switch ($params['action']) {
        ...
        case 'add':
            ...
            if (is_array($userROW)) {
                $mysql->query(
                    "update ".prefix."_eshop_compare set user_id = ".db_squote(
                        $userROW['id']
                    )." where (user_id = 0) and (cookie = ".db_squote($_COOKIE['ngTrackID']).")"
                );
            }

            $mysql->query(
                "insert into ".prefix."_eshop_compare (user_id, cookie, linked_fld) values (".(is_array(
                    $userROW
                ) ? db_squote($userROW['id']) : 0).", ".db_squote($_COOKIE['ngTrackID']).", ".db_squote($id).")"
            );
            ...
            if (count($filter) && is_array(
                    $res = $mysql->record(
                        "select count(*) as count from ".prefix."_eshop_compare where ".join(" or ", $filter),
                        1
                    )
                )
            ) {
            ...
            return array(
                'status' => 1,
                'errorCode' => 0,
                'data' => iconv("windows-1251", "utf-8", 'Item added into compare'),
        ...
        case 'remove':
            ...
            $mysql->query(
                "delete from ".prefix."_eshop_compare where (".join(" or ", $filter).") and linked_fld = ".db_squote(
                    $id
                )." "
            );
            ...
            return array(
                'status' => 1,
                'errorCode' => 0,
                'data' => iconv("windows-1251", "utf-8", 'Item removed from compare'),
                'update' => arrayCharsetConvert(0, $xt->render($tVars)),
            );
...
{
    global $twig, $SYSTEM_FLAGS, $config, $mysql;
    ...
    switch ($params['action']) {
        ...
        case 'do_like':
            ...
            if (!empty($id)) {
                ...
                if (is_array($userROW)) {
                    $mysql->query(
                        "update ".prefix."_eshop_products_likes set user_id = ".db_squote(
                            $userROW['id']
                        )." where (user_id = 0) and (cookie = ".db_squote($_COOKIE['ngTrackID']).")"
                    );
                }
                ...
                if (count($filter) && is_array(
                        $check_like = $mysql->record(
                            "select count(*) as count from ".prefix."_eshop_products_likes where (".join(
                                " or ",
                                $filter
                            ).") AND product_id = ".db_squote($id)." ",
                            1
                        )
                    )
                ) {
                ...
                if ($tCount == 0) {
                    ...
                    $mysql->query(
                        "insert into ".prefix."_eshop_products_likes (user_id, cookie, product_id) values (".(is_array(
                            $userROW
                        ) ? db_squote($userROW['id']) : 0).", ".db_squote($_COOKIE['ngTrackID']).", ".db_squote($id).")"
                    );

                    $mysql->query(
                        "update ".prefix."_eshop_products set likes = likes + 1 where id = ".db_squote($id)." "
                    );

                } else {

                    $mysql->query(
                        "delete from ".prefix."_eshop_products_likes where (".join(
                            " or ",
                            $filter
                        ).") AND product_id = ".db_squote($id)." "
                    );
                ...
                $likes = $mysql->record(
                    "SELECT COUNT(*) as count FROM ".prefix."_eshop_products_likes l WHERE l.product_id='".$id."'"
                );
                ...
                return array(
                    'status' => 1,
                    'errorCode' => 0,
                    'data' => iconv("windows-1251", "utf-8", 'Likes updated'),
                    'update' => arrayCharsetConvert(0, $xt->render($likes_tVars)),
                );
...
{
    global $twig, $ip, $config, $userROW, $mysql;
    ...
    if (empty($error_text)) {
        ...
        $mysql->query(
            "insert into ".prefix."_eshop_products_comments (".implode(",", $vnames).") values (".implode(
                ",",
                $vparams
            ).")"
        );
        ...
        $mysql->query(
            "update ".prefix."_eshop_products set comments = comments + 1 where id = ".db_squote($SQL['product_id'])." "
        );

        $notify_tpath = locatePluginTemplates(
            array('mail/lfeedback_comment'),
            'eshop',
            pluginGetVariable('eshop', 'localsource')
        );
...
{
    global $config, $parse, $TemplateCache, $twig, $mysql;
    ...
    $sqlQPart = "from ".prefix."_eshop_products_comments c LEFT JOIN ".prefix."_users u ON c.author_id = u.id ".(count(
            $conditions
        ) ? "where ".implode(" AND ", $conditions) : '').' '.$fSort;
...
{
    global $mysql, $userROW, $twig, $config;

    // Check if now we're logged in and earlier we started filling ebasket before logging in
    if (is_array($userROW)) {
        $mysql->query(
            "update ".prefix."_eshop_ebasket set user_id = ".db_squote(
                $userROW['id']
            )." where (user_id = 0) and (cookie = ".db_squote($_COOKIE['ngTrackID']).")"
        );
    }
    ...
    $mysql->query(
        "insert into ".prefix."_eshop_ebasket (user_id, cookie, dt, linked_ds, linked_id, title, linked_fld, price, count) values (".(is_array(
            $userROW
        ) ? db_squote($userROW['id']) : 0).", ".db_squote($_COOKIE['ngTrackID']).", ".db_squote(
            $current_time
        ).", ".db_squote($linked_ds).", ".db_squote($linked_id).", ".db_squote($title).", ".db_squote(
            serialize($xfld)
        ).", ".db_squote($price).", ".db_squote($count).") on duplicate key update price=".db_squote(
            $price
        ).", count = count+".db_squote($count)
    );
    ...
    if (count($filter) && is_array(
            $res = $mysql->record(
                "select count(*) as count, sum(price*count) as price from ".prefix."_eshop_ebasket where ".join(
                    " or ",
                    $filter
                ),
                1
    ...
    return array(
        'status' => 1,
        'errorCode' => 0,
        'data' => iconv("windows-1251", "utf-8", 'Item added into ebasket'),
        'update' => arrayCharsetConvert(0, $xt->render($tVars)),
    );
}
...
{
    global $mysql, $userROW, $twig, $config, $ip;
    ...
    $mysql->query(
        "INSERT INTO ".prefix."_eshop_order_basket (`order_id`, `linked_id`, `title`, `count`, `price`, `linked_fld`) VALUES (".db_squote(
            $qid
        ).", ".db_squote($linked_id).", ".db_squote($title).", ".db_squote($count).", ".db_squote(
            $price
    ...
    if ($current_amount != null) {
        if ($current_amount - $r_count > 0) {
            $mysql->query(
                "update ".prefix."_eshop_variants set amount = amount - ".intval($r_count)." where id = ".intval($v_id)
            );
        } else {
...
function basket_update_item_count($id, $linked_ds, $linked_id, $count)
{
    ...
    $mysql->query(
        "update ".prefix."_eshop_ebasket set count = ".db_squote($count)." where (linked_id = ".db_squote(
            $linked_id
        ).") and (linked_ds = ".db_squote($linked_ds).") and (id = ".db_squote($id).")"
    );
...
function basket_delete_item($id, $linked_ds, $linked_id)
{
    ...
    $mysql->query(
        "delete from ".prefix."_eshop_ebasket where id = ".db_squote($id)." and linked_id = ".db_squote(
            $linked_id
        )." and linked_ds = ".db_squote($linked_ds)." limit 1"
    );
...
function basket_update()
{
    ...
    return array(
        'status' => 1,
        'errorCode' => 0,
        'data' => iconv("windows-1251", "utf-8", 'Update ebasket'),
        'update' => arrayCharsetConvert(0, $xt->render($tVars)),
    );
}
...
function ebasket_rpc_manage($params)
{
    ...
    if (!is_array($params) || !isset($params['action'])) {
        return array(
            'status' => 0,
            'errorCode' => 1,
            'errorText' => iconv("windows-1251", "utf-8", 'Параметр действия не задан'),
        );
    }
    ...
    switch ($params['action']) {
        ...
        case 'add':
            $linked_ds = (int)$params['ds'];
            $linked_id = (int)$params['id'];
            $count = (int)$params['count'];
            $variant_id = (int)$params['variant_id'];

            // Check available DataSources
            if (!($DSlist['news'] == $linked_ds)) {
                return array(
                    'status' => 0,
                    'errorCode' => 2,
                    'errorText' => iconv(
                        "windows-1251",
                        "utf-8",
                        'Плагин поддерживает работу только с элементами продукции'
                    ),
                );
            }
            ...
            if ($count < 1) {
                return array(
                    'status' => 0,
                    'errorCode' => 3,
                    'errorText' => iconv("windows-1251", "utf-8", 'Количество продукции должно быть положительным'),
                );
            }
                ...
                case $DSlist['news']:
                    ...
                    if (!is_array($rec)) {
                        return array(
                            'status' => 0,
                            'errorCode' => 4,
                            'errorText' => iconv("windows-1251", "utf-8", 'Продукт с ID ('.$linked_id.') не найден'),
                        );
                    }
                        ...
                        if ($count > $rec['v_amount']) {
                            return array(
                                ...
                                'errorText' => iconv(
                                    "windows-1251",
                                    "utf-8",
                                    'Нет требуемого количества продукта с ID ('.$linked_id.'). Можно заказать только '.$rec['v_amount'].''
                                ),
                            );
                        }
        ...
        case 'update_count':
            $id = (int)$params['id'];
            $linked_ds = (int)$params['linked_ds'];
            $linked_id = (int)$params['linked_id'];
            $count = (int)$params['count'];
        ...
        case 'add_fast':
            ...
            if (empty($order['name'])) {
                return array(
                    'status' => 0,
                    'errorCode' => 6,
                    'errorText' => iconv("windows-1251", "utf-8", 'Не заполнено поле имя'),
                );
            }
            ...
            if (empty($order['phone'])) {
                return array(
                    'status' => 0,
                    'errorCode' => 7,
                    'errorText' => iconv("windows-1251", "utf-8", 'Не заполнено поле телефон'),
                );
            }
            ...
            if (empty($order['address'])) {
                return array(
                    'status' => 0,
                    'errorCode' => 8,
                    'errorText' => iconv("windows-1251", "utf-8", 'Не заполнено поле адрес'),
                );
            }
...
function main_prd($params)
{
    ...
    switch ($params['action']) {
        ...
        case 'show':
            ...
            $tVars['home'] = home;
            $tVars['pagesss'] = generateLP(
                array('current' => $pageNo, 'count' => $countPages, 'url' => '#', 'tpl' => $templateName.'_pages')
            );
