    ...
    if (!empty($result)) {
        ...
        if (empty($order_id) || empty($uniqid)) {
            ...
            if ($row['paid'] == 1) {
                ...
                $merchant_id = $payment_options['merchant_id']; // Р�РґРµРЅС‚РёС„РёРєР°С‚РѕСЂ РјР°РіР°Р·РёРЅР° РІ Pay2Pay
                $secret_key = $payment_options['secret_key']; // РЎРµРєСЂРµС‚РЅС‹Р№ РєР»СЋС‡
                $hash_order_id = $current_time."_".$order_id; // РќРѕРјРµСЂ Р·Р°РєР°Р·Р°
                $amount = $row['total_price']; // РЎСѓРјРјР° Р·Р°РєР°Р·Р°
                $currency = $SYSTEM_FLAGS['eshop']['currency'][0]['code']; // Р’Р°Р»СЋС‚Р° Р·Р°РєР°Р·Р°
                $desc = 'РћРїР»Р°С‚Р° РїРѕ Р·Р°РєР°Р·Сѓ ID: '.$order_id; // РћРїРёСЃР°РЅРёРµ Р·Р°РєР°Р·Р°
                $desc = iconv("windows-1251", "utf-8", $desc);
                $test_mode = $payment_options['test_mode']; // РўРµСЃС‚РѕРІС‹Р№ СЂРµР¶РёРј
