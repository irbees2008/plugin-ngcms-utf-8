    ...
    <table border="0" cellspacing="0" cellpadding="0" class="content" align="center">
        <tr class="contHead" align="left">
            ...
            <td width="5%"><input class="check" type="checkbox" name="master_box"
                                  onclick="javascript:check_uncheck_all(check_comment)"/></td>
        </tr>
        {% for entry in entries %}
            <tr align="left">
                <td width="5%" class="contentEntry1">{{ entry.id }}</td>
                <td width="10%" class="contentEntry1">{{ entry.date|date('d.m.Y H:i') }}</td>
                <td width="20%" class="contentEntry1">{% if (entry.reg) %}<a
                    href="{{ entry.profile_link }}">{{ entry.author }}</a>{% else %}{{ entry.author }}{% endif %}<br/>
                    <small>
                        {{ entry.mail }}
                    </small>&nbsp;
                </td>
                ...
                <td width="5%" class="contentEntry1"><img
                            src="{{ home }}/engine/skins/default/images/{% if (entry.status == 1) %}yes.png{% else %}no.png{% endif %}"
                            alt=""></td>
                <td width="5%" class="contentEntry1"><input name="selected_comment[]" value="{{ entry.id }}"
