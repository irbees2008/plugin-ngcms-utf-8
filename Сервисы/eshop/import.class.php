...
//        $PROP['id'] = (int)$offer->attributes()->id;
        ...
        if ($result) {
            ...
            if ($category_id != 0) {
                ...
                $mysql->query(
                    "DELETE FROM ".prefix."_eshop_products_categories  WHERE `product_id` = '$qid' AND `category_id` = '$category_id' "                       
                );
            ...
            if (isset($price)) {
                ...
                $mysql->query(
                    "INSERT INTO ".prefix."_eshop_variants (`product_id`, `price`, `stock`, `amount`) VALUES ('$qid', '$price', '$stock', '$quantity')"
                );
            }
...
//            }
            ...
            if ($offer->param){
                ...
                foreach ($offer->param as $el) {
                    ...
                    $f_value = iconv('utf-8', 'windows-1251', $el[0]);
                    if ($f_value != "") {
...
//        $PROP['code'] = (string)$offer->vendorCode;
        ...
        if (isset($price)) {
         ...
         $mysql->query(
                        "DELETE FROM ".prefix."_eshop_options  WHERE `product_id` = '$qid'"                       
                    );
