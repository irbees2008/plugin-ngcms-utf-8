...
function plugin_comments_add() {
	...
	if (is_array($addResult = comments_add(array('module' => $module)))) {
		...
		$callingCommentsParams = array('outprint' => true, 'module' => $module);
...
function plugin_comments_show(){
	...
	$callingCommentsParams = array( 'noajax' => 1, 'outprint' => true, 'module' => $module);

	// Try to fetch news
	$newsID = intval($_REQUEST['news_id']);
	$galleryID = intval($_REQUEST['gid']);
	if ($galleryID != 0)
	{
		$galleryRow = $mysql->record("select * from ".prefix."_gmanager where id = ".$galleryID);
		$callingCommentsParams['gallery'] = $galleryID;
		$callingCommentsParams['module'] = 'images';
	}
	switch ($_REQUEST['module'])
	{
		case 'images':
			$module = 'images';
			if ((!$newsID && !$galleryID) || (!is_array($newsRow = $mysql->record("select *, name as title, 1 as allow_com from ".prefix."_images where id = ".$newsID)) && !is_array($newsRow = $mysql->record("select *, 0 as allow_com from ".prefix."_gmanager where id = ".$galleryID)))) {
				error404();
				return;
			}
		break;
	...
	{
		case 'images':
			$tcvars['vars']['link']	= generatePluginLink('gmanager', 'gallery', array('id' => $newsRow['id'], 'name' => $newsRow['name']));
			if ($newsRom['allow_com'] == 0)
			{
				if (pluginGetVariable('gmanager', 'if_description')) $SYSTEM_FLAGS['meta']['description'] = $newsRow['description'];
				if (pluginGetVariable('gmanager', 'if_keywords')) $SYSTEM_FLAGS['meta']['keywords'] = $newsRow['keywords'];
			}
		break;
		default:
...
function plugin_comments_delete(){
	...
	if (!is_array($userROW) || ($userROW['status'] > 2) || ($_GET['uT'] != genUToken(intval($_REQUEST['id'])))) {
		...
		if (($comid) && ($row = $mysql->record("select * from ".prefix."_comments where id=".db_squote($comid)))) {
			...
			$mysql->query("update ".prefix."_".$row['module']." set com=com-1 where id=".db_squote($row['post']));
	...
	if ($_REQUEST['ajax']) {
		...
		// Fetch news record
		if ($row['module'] == 'news' && ($nrow = $mysql->record("select * from ".prefix."_news where id = ".db_squote($params['newsid'])))) {
			$url = newsGenerateLink($nrow);
		} else if ($row['module'] == 'images' && ($irow = $mysql->record("select * from ".prefix."_images where id = ".db_squote($params['newsid'])))) {
			$url = generatePluginLink('gmanager', 'image', array(
					'gallery' => $irow['folder'],
					'id' => $irow['id'],
					'name' => $irow['name']
				)
